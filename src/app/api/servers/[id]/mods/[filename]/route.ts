...
import { deleteMod, excludeMod, includeMod } from "@/lib/services/addon.service";
...
export const DELETE = withAuth(
  async (_req: NextRequest, { session, params }) => {
    try {
      ...
      await deleteMod(params.id, params.filename);
...
export const PATCH = withAuth(
  async (req: NextRequest, { session, params }) => {
    try {
      ...
      if (enabled) {
        await includeMod(params.id, params.filename);
      } else {
        await excludeMod(params.id, params.filename);
      }
